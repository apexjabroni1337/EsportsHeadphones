...
import { allPlayers, proPlayers, headphones } from "@/data";
...
const kbSlug = (name) => { const m = findHeadphone(name); return m ? slug(m.name) : null; };
...
export const metadata = {
  ...
  openGraph: {
    title: "Sensitivity Converter — Convert Sensitivity Between Games",
    description: "Convert your sensitivity between CS2, Valorant, Fortnite, Apex, and more.",
    url: "https://esportsheadphones.com/sensitivity",
    images: [{ url: "https://esportsheadphones.com/og?title=Sensitivity+Converter&subtitle=CS2+%C2%B7+Valorant+%C2%B7+Fortnite+%C2%B7+Apex+%C2%B7+cm%2F360+%C2%B7+Pro+Comparison", width: 1200, height: 630 }],
  },
  twitter: {
    ...
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        mainEntity: [
          ...
          { "@type": "Question", name: "What is eDPI and why does it matter?", acceptedAnswer: { "@type": "Answer", text: "eDPI (effective DPI) = DPI × in-game sensitivity. It normalizes sensitivity across different DPI settings. A player using 400 DPI × 2.0 sens has the same eDPI (800) as someone using 800 DPI × 1.0 sens — they aim at the same speed." }},
      ...
      >
        <h1>Headphone Sensitivity Converter — Convert Settings Between Esports Games</h1>
        <p>
          Convert your sensitivity between {games.length} competitive titles. Our converter calculates
        ...
        <p>
          ...
          calculated as DPI × in-game sensitivity. A player at 800 DPI with 1.0 sensitivity has
      ...
      <SSRSection>
        <SSRTitle accent="#b8956a">Sensitivity</SSRTitle>
        ...
        <SSRGrid>
          ...
          <SSRStat label="Players" value={allPlayers.length.toLocaleString()} color="#b8956a" />
          <SSRStat label="Most Common DPI" value="800" color="#b8956a" />
          <SSRStat label="Pro Profiles" value={proPlayers.length} color="#b8956a" />
        </SSRGrid>
