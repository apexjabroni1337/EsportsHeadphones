...
const BRAND_DESCRIPTIONS = {
  ...
  "G-Wolves": "G-Wolves is a Chinese gaming headphone brand known for the Hati series, which offers lightweight headphones with aggressive specifications at budget-friendly prices. The Hati-M and Hati-S variants provide different size options for competitive players, and the brand has developed a following among enthusiasts who appreciate the value proposition.",
  ...
  EPOS: "EPOS is a premium Danish audio company that emerged from the legendary Sennheiser Communications joint venture between Sennheiser and Demant. When the partnership concluded in 2020, EPOS inherited decades of Sennheiser audio engineering expertise and dedicated it to gaming and enterprise communication. The H6PRO series became EPOS's flagship esports headphone, available in both open and closed acoustic variants — a dual-design philosophy unique in competitive gaming that lets players choose between wide soundstage for spatial awareness or sealed isolation for noisy LAN environments. The GSP 500 remains a trusted choice among professional Counter-Strike players who value its open-back design for natural sound reproduction and precise positional audio. EPOS headphones are distinguished by their lift-to-mute magnetic boom microphone system, studio-grade transducers tuned specifically for gaming communication clarity, and Scandinavian industrial design that prioritizes comfort and durability. The brand's commitment to audio quality over flashy features has earned EPOS a loyal following among esports professionals who treat their headphones as serious competitive tools rather than accessories.",
};
...
export function generateMetadata({ params }) {
  ...
  return {
    ...
    description,
    alternates: { canonical: `https://esportsheadphones.com/brands/${params.slug}` },
    openGraph: {
      title: `${brand} Esports Headphones — All Models, Pro Usage & Reviews`,
      description,
    ...
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Brands", item: "https://esportsheadphones.com/brands" },
          { "@type": "ListItem", position: 3, name: brand, item: `https://esportsheadphones.com/brands/${params.slug}` },
        ],
      }) }} />
      ...
      >
        <h1 itemProp="name">{brand} Esports Headphones — Complete Lineup, Pro Usage, and Reviews</h1>
        <meta itemProp="url" content={`https://esportsheadphones.com/brands/${params.slug}`} />
        ...
        <h2>Complete {brand} Headphone Lineup</h2>
        <table>
          <caption>All {brand} esports headphones — specifications, pricing, and pro usage</caption>
          <thead>
        ...
        {brandPros.length > 0 && (
          <>
            <h2>Professional Players Using Headphones</h2>
            <p>{brandPros.length} professional players use {brand} headphones across {brandGames.length} games: {brandGames.join(", ")}.</p>
            <table>
              <caption>Pro players using {brand} headphones</caption>
              <thead>
                <tr><th>Player</th><th>Game</th><th>Team</th><th>Headphone</th></tr>
              </thead>
              <tbody>
        ...
        <h2>{brand} Headphone Comparison</h2>
        <p>
          The {brand} esports headphone lineup ranges from {weightRange} in weight and {priceRange} in price.
        ...
        <nav>
          ...
          <ul>
            ...
            {brandGames.map((g) => (
              <li key={g}><a href={`/games/${slug(g)}`}>{g} Headphone Usage</a></li>
            ))}
          </ul>
        </nav>
      </article>
