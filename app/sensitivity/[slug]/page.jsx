...
function getGameData(gameName) {
  ...
  return { players, dpis, edpis, avgDpi, avgEdpi, medianEdpi, minEdpi, maxEdpi, topHeadphones, dpiDist };
}
...
export function generateMetadata({ params }) {
  ...
  const description = `${info.fullName} pro player headphone settings — ${players.length} players tracked. Find pros using the same headphones and gear.`;

  return {
    title: `${info.fullName} Pro Sensitivity Settings`,
    description,
    alternates: { canonical: `https://esportsheadphones.com/sensitivity/${params.slug}` },
    openGraph: {
      title: `${info.fullName} Pro Sensitivity Settings`,
      description,
      url: `https://esportsheadphones.com/sensitivity/${params.slug}`,
      images: [{ url: `https://esportsheadphones.com/og?title=${encodeURIComponent(info.fullName + ' Sensitivity')}&subtitle=${encodeURIComponent(`${players.length} Pro Players · Headphone & Settings Guide`)}`, width: 1200, height: 630 }],
    },
...
export default function SensitivityGamePage({ params }) {
  ...
  const info = GAME_SENS_INFO[gameName];
  const { players, avgDpi, avgEdpi, medianEdpi, minEdpi, maxEdpi, topHeadphones } = getGameData(gameName);
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "@type": "Article",
        headline: `${info.fullName} Pro Player Headphone and Sensitivity Guide`,
        description: `Complete guide to ${info.fullName} headphone settings and sensitivity. ${players.length} pro players tracked with detailed headphone preferences.`,
        url: `https://esportsheadphones.com/sensitivity/${params.slug}`,
      ...
      >
        <h1>{info.fullName} Pro Player Headphone Settings and Gear</h1>

        <h2>About {info.fullName}</h2>
        <p>{info.notes}</p>
        <p>
          We track {players.length} professional {info.fullName} players and their complete headphone setups.
        </p>

        <h2>Top {info.fullName} Pro Players</h2>
        <table>
          <caption>Professional {info.fullName} player headphone settings</caption>
        ...
        {topHeadphones.length > 0 && (
          <>
            <h2>Most Popular Headphones in {info.fullName}</h2>
            <table>
              <caption>Most used gaming headphones among professional {info.fullName} players</caption>
              <thead><tr><th>Headphone</th><th>Players</th></tr></thead>
              <tbody>
                {topHeadphones.map(([m, count]) => (
                  <tr key={m}><td><a href={`/headphones/${sl(m)}`}>{m}</a></td><td>{count}</td></tr>
                ))}
              </tbody>
            </table>
          </>
